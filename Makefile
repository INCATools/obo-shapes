...
all: validate-shex validate-shacl run-all-shex run-all-shacl

.PHONY: all validate-shex validate-shacl run-all-shacl run-all-shex clean
...
output/%-shacl.out: test-data/% shacl/obo-shapes.ttl
	mkdir -p $(dir $@)
	shacl v --shapes shacl/obo-shapes.ttl --data $< --text >$@

output/%-shex.out: test-data/% shex/obo-shapes.shex shex/obo-shapes.shapeMap
	mkdir -p $(dir $@)
